...
const Head = () => {
  ...
  return (
    <div className='grid grid-flow-col py-3 shadow-xl'>
    <div  className="flex col-span-1 px-6">
       <img 
       onClick={()=>toggleMenuHandler()}
       className="h-12 m-2 cursor-pointer" 
       src='https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSnAFlEOJtVdN92jf795u1d5OlHBYTcQAQFGw&usqp=CAU' 
        alt="menu" 
        />
        <a href="/">
    ...
    <div>
    <div className='col-span-10 m-2 mr-3 px-2'>
        <input 
        value={searchQuery}
        onChange={(e)=>setSearchQuery(e.target.value)}
        onFocus={()=>setShowSuggetion(true)}
        onBlur={()=>setShowSuggetion(false)}
        className='px-3 w-[500px] h-10 ring-2 ring-gray-400 rounded-l-full' type="text" />
        <button className='rounded-r-full ring-2 ring-gray-400 h-10 py-2 px-5 bg-gray-300'>
           🔍
          </button>
    </div>
    {showSuggetion &&
     <div className="absolute bg-white ml-5 px-5 py-2 w-1/5 shadow-lg rounded-lg border-gray-200">
      <ul>
        {suggetion.map((s)=>(
        <li key={s} className='p-2 shadow-sm'>🔍 {s}</li>
        ))}
      </ul>
     </div>
}
    ...
    <div className='col-span-1'>
      <img className='h-12 m-2 float-right'
